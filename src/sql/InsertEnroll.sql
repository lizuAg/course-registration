...
/* 에러 처리 1 : 최대학점 초과여부*/
SELECT SUM(c.c_unit) INTO nSumCourseUnit FROM course c, enroll e
WHERE e.m_id = sStudentId and e.e_year = nYear and 
e.e_semester = nSemester and e.c_id = c.c_id and e.c_id_no = c.c_id_no;
SELECT c_unit INTO nCourseUnit FROM course
...
SELECT COUNT(*) INTO nCnt FROM enroll WHERE m_id = sStudentId and c_id = sCourseId;
IF (nCnt > 0) 
	THEN
...
SELECT t_max INTO nTeachMax FROM teach
WHERE t_year= nYear and t_semester = nSemester and c_id = sCourseId and c_id_no= nCourseIdNo;
SELECT COUNT(*) INTO nCnt FROM enroll WHERE e_year = nYear and e_semester = nSemester
...
WHERE e.m_id=sStudentId and e.e_year=nYear and e.e_semester = nSemester
and
...
/* 수강 신청 등록 */
INSERT INTO enroll(M_ID,C_ID,C_ID_NO,E_YEAR,E_SEMESTER)
